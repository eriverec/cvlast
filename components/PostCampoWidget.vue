  ...
  <div class="blog-post">
    <div class="style_root__3iCRH">
      <div class="style_rootInner__32CX1">
        <!-- <prismic-image
            v-if="postcampo.data && postcampo.data.image"
            :field="postcampo.data.image"
            sizes="(max-width: 990px) 100vw (min-width: 991px) 57vw"
          /> -->
        <div class="style_title__1jQC3">
          <h2>{{ $prismic.asText(postcampo.data.title) }}</h2>
        </div>
        <prismic-rich-text :field="postcampo.data.texto" class="parrafo" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  name: "post-campo-widget",
  methods: {
    // Function to get the first paragraph of text in a blog post and limit the displayed text at 300 characters
    getFirstParagraph(postcampo) {
      const textLimit = 300;
      const slices = postcampo.data.body;
      let firstParagraph = "";
