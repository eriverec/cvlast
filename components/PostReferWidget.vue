    ...
    <div class="style_root__3iCRH">
      <div class="style_rootInner__32CX1">
        <!-- <prismic-image
            ...
            :field="postrefer.data.image"
            sizes="(max-width: 990px) 100vw (min-width: 991px) 57vw"
          /> -->
        <div class="style_title__1jQC3">
          <h2>{{ $prismic.asText(postrefer.data.title) }}</h2>
        </div>
        <prismic-rich-text :field="postrefer.data.texto" class="parrafo" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  name: "post-refer-widget",
  methods: {
    // Function to get the first paragraph of text in a blog post and limit the displayed text at 300 characters
    getFirstParagraph(postrefer) {
      const textLimit = 300;
      const slices = postrefer.data.body;
      let firstParagraph = "";
